...
python3 semantic_scholar_fetch.py search "semantic communication" --max 10 \
...
python3 semantic_scholar_fetch.py search-bulk "semantic communication" --max 50 \
  --sort citationCount:desc --fields-of-study "Computer Science" --year "2020-"

# Fetch a single paper by DOI or arXiv ID
python3 semantic_scholar_fetch.py paper "10.1109/JSAC.2021.3126077"
python3 semantic_scholar_fetch.py paper "ARXIV:2006.10685"
...
def _require_api_key() -> None:
    """Ensure a valid Semantic Scholar API key is configured before running.

    A valid key must start with ``s2k-`` and be longer than 40 characters.
    """
    api_key = os.getenv("SEMANTIC_SCHOLAR_API_KEY", "").strip()
    if not (api_key.startswith("s2k-") and len(api_key) > 40):
        raise RuntimeError(
            "SEMANTIC_SCHOLAR_API_KEY is " + ("not set" if len(api_key) == 0 else "invalid")
        )
